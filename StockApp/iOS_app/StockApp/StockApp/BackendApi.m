...
+ (void)initNetworkConnection:(runBlock)block{
    ...
    CFStreamCreatePairWithSocketToHost(NULL, (CFStringRef)@"192.168.0.8", 8080, &readStream, &writeStream);
...
+ (void)stream:(NSStream *)theStream handleEvent:(NSStreamEvent)streamEvent {
    ...
    switch (streamEvent) {
        ...
        case NSStreamEventHasSpaceAvailable: {
            ...
            NSLog(@"Stream is avalaible to write");
        ...
        case NSStreamEventHasBytesAvailable:
            ...
            NSLog(@"Stream is available to read");
        ...
        case NSStreamEventErrorOccurred:
            ...
            NSLog(@"%@", [[theStream streamError] debugDescription]);
...
+ (NSString *)readString{
    const size_t BUFFER_SIZE = 1024; // 1,024
    
    uint8_t buffer[BUFFER_SIZE]; // avoid reallocation
...
+ (BOOL)setUpAccount:(NSString *) username withPassword:(NSString *) password {
    ...
    NSString* messageToSend = [NSString stringWithFormat:@"%c%c%@%c%@\r\n", routine, usernameSize, username, passwordSize, password];
